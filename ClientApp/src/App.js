...
export default class App extends Component {
  ...
  updateCar = (vin, status) => {
    ...
    this.putData(copyData.filter(car => car.vin === vin)[0], `http://${this.props.connection}:3001/putcar`);
  }
  ...
  addCar = (car) => {
    ...
    this.postData(car, `http://${this.props.connection}:3001/postcar`)
  };
  ...
  addCarRule = (rule) => {
    ...
    this.postData(rule, `http://${this.props.connection}:3001/postrule`)
  };
  ...
  addMessage = (message) => {
    ...
    this.postData(message, `http://${this.props.connection}:3001/postmessage`)
  }
  ...
  deleteCarRule = (rule) => {
    ...
    this.deleteData(rule, `http://${this.props.connection}:3001/deleterule`)
  }
  ...
  componentDidMount() {
    this.getData(`http://${this.props.connection}:3001/`).then((data) => {
      this.setState({ ...this.state, inventoryData: data, loading: false });
    });
    this.getData(`http://${this.props.connection}:3001/getusers`).then((data) => {
      this.setState({ ...this.state, userData: data, loading: false });
    });
    this.getData(`http://${this.props.connection}:3001/getrules`).then((data) => {
    ...
    this.getData(`http://${this.props.connection}:3001/getmessages`).then((data) => {
